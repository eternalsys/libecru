...
type User struct {
	...
	Authority   string    `form:"authority" boil:"authority" json:"authority" toml:"authority" yaml:"authority"`
...
var UserWhere = struct {
	...
	Authority:   whereHelperstring{field: "`users`.`authority`"},
...
var (
	userAllColumns            = []string{"id", "password", "authority", "last_name", "first_name", "mail_address", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"id", "password", "authority", "last_name", "first_name", "mail_address"}
	userColumnsWithDefault    = []string{"created_at", "updated_at"}
	userPrimaryKeyColumns     = []string{"id", "password"}
)
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, iD string, password string, selectCols ...string) (*User, error) {
	userObj := &User{}
	...
	query := fmt.Sprintf(
		"select %s from `users` where `id`=? AND `password`=?", sel,
	)

	q := queries.Raw(query, iD, password)
...
func (o *User) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `users` WHERE `id`=? AND `password`=?"
...
func (o *User) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUser(ctx, exec, o.ID, o.Password)
	if err != nil {
		return err
	}
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, iD string, password string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `users` where `id`=? AND `password`=? limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, iD, password)
	}
	row := exec.QueryRowContext(ctx, sql, iD, password)

...
func testUsersExists(t *testing.T) {
	...
	e, err := UserExists(ctx, tx, o.ID, o.Password)
	if err != nil {
...
func testUsersFind(t *testing.T) {
	...
	userFound, err := FindUser(ctx, tx, o.ID, o.Password)
	if err != nil {
		t.Error(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `varchar`, `Password`: `varchar`, `Authority`: `varchar`, `LastName`: `varchar`, `FirstName`: `varchar`, `MailAddress`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_           = bytes.MinRead
)
